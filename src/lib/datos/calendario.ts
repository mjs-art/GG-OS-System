...
export async function entradasDelMes({
  ...
  const [clientesRes, pilaresRes, piezasRes, storiesRes] = await Promise.all([
  ...
  const primerError = clientesRes.error ?? pilaresRes.error ?? piezasRes.error ?? storiesRes.error
  if (primerError) {
